...
test("upload process — login + navigate + autosave + double-click", async ({ page }) => {
  await page.goto("http://localhost:8080/auth/login");
  await page.getByLabel(/e-?mail/i).fill(EMAIL);
  await page.getByLabel(/senha/i).fill(PASS);
  await page.getByRole("button", { name: /entrar|login/i }).click();
  await page.waitForURL(/dashboard/i, { timeout: 15000 });
  await page.screenshot({ path: "/tmp/browser/4c1/screenshots/upload_dashboard.png" });
  await page.goto("http://localhost:8080/processes");
  await page.screenshot({ path: "/tmp/browser/4c1/screenshots/upload_processes.png" });
