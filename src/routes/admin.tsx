...
function AdminLayout() {
  ...
  const adminNavItems = [
    ...
    { name: "Modelos de Documentação", icon: <FileText className="h-5 w-5" />, path: "/admin/templates" },
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      <aside className={`${isSidebarOpen ? "w-72" : "w-20"} transition-all duration-500 bg-[#020D1D] text-white flex flex-col z-50 border-r border-white/5`}>
        ...
        <nav className="flex-grow mt-6 px-4 space-y-2 overflow-y-auto custom-scrollbar">
          {adminNavItems.map((item) => (
            <Link key={item.name} to={item.path} activeProps={{ className: "bg-primary text-white shadow-lg shadow-primary/20" }} className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/5 transition-all group">
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-20">
      <PageHeader 
        ...
        description="Gestão global de infraestrutura, clientes e performance financeira."
        actions={<Badge className="bg-primary text-white border-none font-black uppercase text-[10px] tracking-widest py-2 px-4">Admin Global Ativo</Badge>}
      />
                ...
                {[1, 2, 3].map((_, i) => (
                  <div key={i} className="p-6 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                    <div className="flex items-center gap-4">
                       ...
                       <div><p className="text-sm font-bold text-navy">Tenant Corporativo #{i+1}</p><p className="text-[10px] text-slate-400 font-bold uppercase">Última atividade: há 2 horas</p></div>
                    </div>
